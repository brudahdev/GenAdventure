/**
 * Client-side audio input service. Streams microphone audio to the Voxta server
 * over a WebSocket opened directly from the renderer (getUserMedia/MediaRecorder
 * are renderer-only). The session id, base URL, and apiKey arrive transiently in
 * the recording-start IPC event — the apiKey is held in a closure here and never
 * logged or persisted.
 *
...
/** Log helper — kept terse and apiKey-free. */
...
export class AudioInputService {
     ...
     *  server requests recording. Cleared by dispose(). */
    ...
    /**
     * Handle recording request from server.
    ...
    dispose(): void {
        this.stopStreaming();
        // Clear the permanent-failure flag so a fresh service instance can retry.
