import type { SpatialPosition, AudioEffectSettings } from './audio-types';

/**
 * Spatial audio engine using the Web Audio API.
 * Provides distance-based volume attenuation, stereo directional panning,
 * cave-like reverb, and echo effects for the character's voice.
 *
 * NOTE: GenAdventure does not yet feed it position/settings data — the chat
 * controller just calls playChunk(), so the spatial/reverb/echo effects sit at
 * their inert constructed defaults (spatial disabled, reverb/echo at 0). The
 * machinery is intact for a future change that wires up updatePosition/applySettings.
 */
export class SpatialAudioEngine {
    ...
    private lastSettings: AudioEffectSettings | null = null;
    ...
    /** Apply settings from AudioEffectSettings */
    applySettings(settings: AudioEffectSettings): void {
